...
class ArtikelController extends Controller
{
    ...
    public function saveDataArtikel(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['toast_message' => 'Data Artikel Berhasil Ditambah', 'artikel' => $artikel], 201);
        } catch (\Exception $e) {
            \Log::error($e); // Log the error
            return response()->json(['toast_message' => 'Terjadi kesalahan saat menyimpan data.'], 500);
        }
    }
    ...
    public function updateDataArtikel(Request $request, $id)
    {
        try {
            ...
            return response()->json(['success' => true, 'toast_message' => 'Data Artikel Berhasil Diubah']);
        } catch (\Exception $e) {
            return response()->json(['success' => false, 'toast_message' => 'Gagal memperbarui data: ' . $e->getMessage()]);
        }
    }
    ...
    public function hapusDataArtikel($id)
    {
        $artikel = Artikel::find($id);
        if (!$artikel) {
            return response()->json(['toast_message' => 'Artikel tidak ditemukan.'], 404);
        }
