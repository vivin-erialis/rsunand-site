...
class ArtikelController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Unggah gambar ke penyimpanan eksternal (misalnya, Amazon S3)
        $gambarPath = $request->file('gambar')->store('images/artikel', 's3');

        // Dapatkan URL gambar dari penyimpanan eksternal
        $gambarUrl = Storage::disk('s3')->url($gambarPath);

        $artikel = Artikel::create([
            'title' => $request->title,
            'desc' => $request->desc,
            'isi' => $request->isi,
            'gambar' => $gambarUrl, // Simpan URL gambar, bukan nama file
            'status' => '1',
            'url' => $slug,
