...
class TentangKamiController extends Controller
{
    ...
    public function saveProfile(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'telp' => 'required', // Example regex for phone number
        ]);
        ...
        try {
            // Find the user profile, assuming the user is authenticated
            $user = Auth::user();
            $profile = $user->profile; // Assuming there is a one-to-one relationship with Profile

            // Update the profile with the request data
            $profile->sejarah = $request->input('sejarah');
            $profile->email = $request->input('email');
            $profile->telp = $request->input('telp');
            $profile->save();
