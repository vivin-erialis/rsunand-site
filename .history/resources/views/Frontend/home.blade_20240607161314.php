                ...
                <div class="col-lg-4">
                    ...
                    @foreach ($recentPosts as $recent)
                        <div class="recent-post mb-3">
                            @if ($recent->gambar)
                                @php
                                    $gambarArray = json_decode($recent->gambar);
                                    if ($gambarArray) {
                                        $gambarPertama = reset($gambarArray);
                                    }
                                @endphp
                                ...
                                @if (!empty($gambarPertama))
                                    <img loading="lazy" class="img-fluid mb-2"
                                        src="{{ asset('/../images/artikel/' . $gambarPertama) }}" alt="{{ $recent->title }}">
                                @endif
                            @endif
                            <div>
                                <a href="/berita/{{ $recent->url }}">
                                    <h5 class="mb-1">{{ $recent->title }}</h5>
                                </a>
                                <span class="text-muted" style="font-size: 12px">
                                    <i class="far fa-calendar me-2"></i>{{ Carbon\Carbon::parse($recent->created_at)->isoFormat('D MMMM, Y') }}
                                </span>
                            </div>
                        </div>
                    @endforeach
