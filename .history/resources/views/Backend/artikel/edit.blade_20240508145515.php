            ...
            <div class="col-12">
                <div class="card border shadow-xs mb-4">
                    ...
                    <div class="card-header">
                        <form role="form" action="/admin/artikel/{{$artikel->id}}" method="post" enctype="multipart/form-data">
                            @method('PUT')
                            @csrf
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="kategori_id" id="mySelect" class="form-control">
                                            @foreach ($kategori as $item)
                                                <option value="{{ $item->id }}" {{ $item->id == $dataLama->kategori_id ? 'selected' : '' }}>{{ $item->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
