    ...
    <div class="container-fluid py-3 px-3">
                    ...
                    <div class="card-header border-bottom pb-0 mb-3">
                        <div class="d-sm-flex align-items-center">
                            ...
                            <div class="ms-auto d-flex">
                                <button class="btn btn-sm btn-dark btn-icon d-flex align-items-center" data-bs-toggle="modal"
        ...
        <script>
            $(document).ready(function() {
                ...
                // Fungsi untuk menampilkan toast dengan pesan khusus
                function showToast(title, message, delay = 5000) {
                    // Clone template toast
                    var $toast = $('#toast-template').clone().removeAttr('id').appendTo('.toast-container');

                    // Set judul dan pesan toast
                    $toast.find('#toast-title').text(title);
                    $toast.find('#toast-body').text(message);

                    // Initialize Bootstrap toast
                    var toast = new bootstrap.Toast($toast[0], { delay: delay });

                    // Tampilkan toast
                    toast.show();

                    // Hapus toast dari DOM setelah tersembunyi
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            response.artikel.forEach(function(article) {
                                ...
                                html += '<td>';
                                html += '<button class="btn btn-sm btn-success edit-btn" data-id="' + article.id + '" data-bs-toggle="modal" data-bs-target="#editModal"> <i class="fa fa-edit text-xs me-2"></i> Edit</button>';
                                html += '<button class="btn btn-sm btn-danger delete-btn" data-id="' + article.id + '" data-bs-toggle="modal" data-bs-target="#deleteModal"> <i class="fa fa-trash text-xs me-2"></i> Hapus</button>';
                                html += '</td>';
                                html += '</tr>';
                            });
                ...
                $('#artikelForm').on('submit', function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            showToast('Sukses', 'Data artikel berhasil disimpan!');
                        },
                    ...
                    $('#deleteBtn').off('click').on('click', function() {
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                showToast('Sukses', 'Data artikel berhasil dihapus!');
                            },
