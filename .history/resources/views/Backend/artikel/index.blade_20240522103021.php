...
@section('content')
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            response.artikel.forEach(function(article) {
                                ...
                                if (article.status == 0) {
                                    html +=
                                        '<a href="" class="btn btn-sm btn-success"></i>Aktif</a>';
                                } else {
                                    html +=
    ...
    $.ajax({
        url: '/admin/artikel/' + articleId + '/change-status',
        type: 'PUT',
        data: {
            status: newStatus
        },
        ...
        success: function(response) {
            ...
            if (newStatus == 1) {
                $(this).removeClass('btn-success').addClass('btn-danger').text('Non Aktif').data('status', 0);
            } else {
                $(this).removeClass('btn-danger').addClass('btn-success').text('Aktif').data('status', 1);
            }
