        ...
        <script>
            $(document).ready(function() {
                ...
                $(document).on('click', '.edit-btn', function() {
                    ...
                    $.ajax({
                        url: '/admin/artikel/' + dataId + '/edit',
                        type: 'GET',
                        success: function(response) {
                            // Isi formulir modal dengan data artikel yang diterima dari server
                            $('#dataId').val(response.id);
                            $('#judulArtikel').val(response.nama);
                            $('#judulArtikel').val(response.nip);
                            $('#judulArtikel').val(response.tempat_lahir);
                            $('#judulArtikel').val(response.tanggal_lahir);
                            $('#deskripsiArtikel').val(response.pendidikan);

                            // Atur nilai menggunakan metode setData dari CKEditor setelah CKEditor sepenuhnya diinisialisasi
                            if (window.editor2) {
                                window.editor2.setData(response.pendidikan);
                            }

                            $('#kategoriArtikel').val(response.spesialis_id);
